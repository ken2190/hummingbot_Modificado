...
def print_profit_opportunity_for_path_multi(graph: nx.Graph, path, print_output=True, round_to=None, shorten=False, money=100):
    """
    ...
    for i in range(len(path)):
        if i + 1 < len(path):
            ...
            if not shorten:
                ...
                if str(graph[start][end]['market_name']).index(start)<str(graph[start][end]['market_name']).index(end):
                    vectorTransactions.append((str(start), str(end), float(rate), float(money),
                                           str(graph[start][end]['exchange_name']),
                                           str(graph[start][end]['market_name']),"sell"))
                elif str(graph[start][end]['market_name']).index(start)>str(graph[start][end]['market_name']).index(end):
                    vectorTransactions.append((str(start), str(end), float(rate), float(money),
                                           str(graph[start][end]['exchange_name']),
                                           str(graph[start][end]['market_name']),"buy"))
