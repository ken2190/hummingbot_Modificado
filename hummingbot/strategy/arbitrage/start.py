...
from arbitrageCore.arbitrage_multiple_exchanges_multiple_coins import transaccionesOptimas


def start(self):
    '''hola'''
    '''[('TRX', 'DAI', 1, 135, 'kraken', 'TRX/EUR', 'buy')]'''
    monto=100
    vectorDeTransacciones,profit =buscarMejorCaminoDeTradesYEjecutarlo(['binance','kraken','bitrex'],moneda,monto,False)
    for x in range(0,len(vectorDeTransacciones)-1):
            
        primary_market = vectorDeTransacciones[x][4].lower()
        secondary_market = vectorDeTransacciones[x+1][4].lower()
        raw_primary_trading_pair = vectorDeTransacciones[x][5].replace("/","")
        raw_secondary_trading_pair = vectorDeTransacciones[x+1][5].replace("/","")
